...
EXCLUDE_PERIODS = ['202310','202311', '202312']
...
def process_file(file_path):
    # try:
        ...
        df['对应费款所属期'] = df['对应费款所属期'].astype(str)
        df['社会保障号'] = df['社会保障号'].astype(str)
        # df = df[~df['对应费款所属期'].isin(EXCLUDE_PERIODS)]
        df['对应费款所属期'] = df['对应费款所属期'].str[:4]
        grouped_df = df.groupby(['姓名', '社会保障号', '对应费款所属期'])['个人缴费'].sum().round(2).reset_index()
        return grouped_df
...
def build_new_row(grouped_df):
    ...
    for row in grouped_df.itertuples():
        ...
        if row.对应费款所属期 in YEARS:
            new_row[f'{row.对应费款所属期}缴费'] = row.个人缴费
    return new_row

def main():
    totalDf = initialize_dataframe()
    print("请确保文件表头中有以下字段：姓名，社会保障号，对应费款所属期，个人缴费等名称")
    print("统计维度是按照 姓名，社会保障号，和对应费款所属期进行统计的")
    ...
    for file_name in os.listdir(FOLDER_PATH):
        if file_name.endswith(".xlsx") or file_name.endswith(".xls"):
            ...
            if grouped_df is not None:
                ...
                pivot_df = grouped_df.pivot_table(index=['姓名', '社会保障号'], columns='对应费款所属期', values='个人缴费',
